...
- **There is no L2 light** (M5 as built: `Params.Lights` is S1–S4, L1, L4; L2 is chart 3's promise). Where this plan says "L2 goes red", the panel in fact shows L1 red in the band run (the shared workers raise the healthy p99, above) and L4 red in the global run; L1 is not judged in either global surge (every tenant is offered load, so no tenant is unaffected; M5 › Status), and the healthy-p99 tile shows the rise instead. M5's every-scenario script allows exactly those reds and requires green once each tail has ended.
