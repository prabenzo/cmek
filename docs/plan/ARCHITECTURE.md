...
`internal/cmek` imports exactly: `context`, `crypto/aes`, `crypto/cipher`, `crypto/rand`, `encoding/binary`, `errors`, `fmt`, `io`, `sync`, `sync/atomic`, `time`, `golang.org/x/sync/singleflight`, `github.com/prabenzo/cmek/internal/kms`. Enforced in CI by `go list -deps ./internal/cmek | grep -vE '^(golang.org/x/sync|github.com/prabenzo/cmek/internal/(cmek|kms)$|[a-z/]+$)'` producing no output (no `TestImports` in the package) `[BB-04]`. `queue` does not import `traffic`; `metrics` does not import `cmek` (world bridges the audit type in 15 lines). From M1 on, `world.keys` is `*cmek.Manager`; there is no key stub outside `internal/cmek` `[BB-03]`.
...
- **Manager locking.** Recommended: one `m.mu` (every section is µs) with a lock-free `states` atomic array for the grid so the metrics tick never enters it. Alternative: per-tenant mutexes as in proposal A.
