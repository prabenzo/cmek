...
| M3 Admission and surges | 1:50–2:20 | [M3.md](M3.md) | [x] 2026-09-22, chat | 90 s global surge and every other decision as recommended; Claude writes all code and tests, Ben reviews; build starts once M2 merges |
...
| M5 Checker and hardening | 3:15–3:40 | [M5.md](M5.md) | [x] 2026-09-23, chat | all as recommended (six lights, two revocation lines, `StreamMaxAge` reconnect, holder grown in place, `TestCheckerTurnsRed` in the box with the storm and idle-rule tests); Claude writes everything incl. `holder.go`, reset and the README; Ben deploys, runs the clean-browser check, reviews |
...
| No key cache (sixth scenario, before M5) | — | [NOCACHE.md](NOCACHE.md) | [x] 2026-09-23, chat ("implement it") | Ben's ask (2026-09-23): a failure mode that shows what happens with no caching, every seal and open calling through to the KMS; one card, two runs: one provider's band without the cache next to two cached ones at a realistic KMS latency with the gcp blip in the middle, and everyone without the cache under the global surge (Ben's follow-up: "what happens if we designed this without caching and got popular"); nine decisions, all with a recommendation |
...
| M4: L1 floor, L4 tolerance, tenant-surge card wording, hover/pin and the ordering deviations, p99 quantization, revocation restore, revoke target rank, `#inflight` wording | M4 | all as recommended for the build; "most of the decisions are around UI, which I will review during my testing" (browser review at Ben's pickup) | chat, 2026-09-22 |
...
| No key cache: two runs on one card, the global run's affected set (the 150 heaviest, as the cached surge) and multiplier (5×, 60 s), latency on every provider, tenant cap kept, L2 (and L1/L4) red with the card saying so, scenario-only switch, backoff kept for recovery, phase lengths | NOCACHE | all as recommended ("implement it") | chat, 2026-09-23 |
| M5: C1 (L1/L4 in the box), revocation timeline lines, stream lifetime, holder phasing, tests | M5 | all as recommended: six lights, two lines per episode, `StreamMaxAge = 55 min` + `event: reconnect`, `holder.go` grown in place, `TestCheckerTurnsRed` required with the storm and idle-rule tests built alongside it; "M5 plan LGTM, start the implmentation" | chat, 2026-09-23 |
