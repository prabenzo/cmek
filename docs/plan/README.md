...
3. Read `M0.md` through `M6.md` in order. Each follows one template: goal, cut line, dependencies, files with line counts and owner, interfaces, order of work with a running clock in two lanes ([C] Claude in the session, [B] Ben on his laptop), tests, acceptance check, decisions for Ben, risks, and a review log of the findings that shaped it. Library-, struct- and timeout-level questions live here, not in the architecture document.
