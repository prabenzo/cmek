...
- **Ben's decisions are per milestone**, about 50 in all (indexed in `COVERAGE.md § 11`). Each bullet gives a recommended option, the alternative, and what changes in the plan if the alternative is chosen. Only M0's three (deploy path, cross-compiling builder stage, tick burn) are needed before the clock starts; M1's first one (nonce source) is needed by 0:22.
...
1. Read `ARCHITECTURE.md` once: package layout, `World` and `Params`, the exact call sequences, SQL, the per-package exported surface, the concurrency rules, and answers to twenty hard design questions. Its "Amendments" section at the top lists where the milestone plans overrode it. Its last two appendices (rejected judge findings, review log) are the planning record and can be skipped.
2. Read `ARCH-NOTES.md` (nine facts and corrections the plans cite by number, such as the exact Cloud Run command and the Go 1.26 requirement) and, if you want the brief the planners worked from, `CONTEXT.md`.
3. Read `M0.md` through `M6.md` in order. Each follows one template: goal, cut line, dependencies, files with line counts and owner, interfaces, order of work with a running clock in two lanes ([C] Claude in the session, [B] Ben on his laptop), tests, acceptance check, decisions for Ben, risks, and a review log of the findings that shaped it.
