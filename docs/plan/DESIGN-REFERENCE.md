...
Fake call order (the KEK gate's `EncryptWithContext`/`DecryptWithContext`, Tink as built): resolve fault under `fake.mu` (tenant fault overrides provider fault field-by-field where set) → truth log call start → `FastFail || rand < ErrorRate` → `kms.Error{Unavailable}` at once → else sleep lognormal: `μ = ln(P50)`, `σ = (ln(P99) − ln(P50)) / 2.326`, `d = exp(μ + σ·Z)` with Z from the locked rand, via `select { <-clock.After(d); <-ctx.Done() → return ctx.Err() }` → key enabled? checked **after** the latency under `RLock` → `kms.Error{KeyDisabled}` → real AES-256-GCM wrap/unwrap of the DEK keyset under the tenant KEK (a Tink keyset made at `NewFake`, AAD = KEK id; a blob that does not verify answers `AccessDenied`) → truth log outcome.
