...
| Key-fetch modes (as built) | [KEYFETCH.md](KEYFETCH.md), approved 2026-09-24, replaces the scripted `slow_kms_naive` run: `cmek.Fetch` (`FetchAsync`, `FetchSync`, `FetchPassThrough`, `FetchNaive`) replaces the tenant's two booleans, `Manager.SetFetch(id, mode)` with `SetPassThrough`/`SetNaive` as wrappers; in `FetchSync` nothing is kicked (`EncryptKey`'s hot path, `Hot`, `Tick` skip the probe spawn), `DecryptKey` runs the soft-due renewal, the due probe of a parked tenant or the unwrap of a cold DEK inline before its checks, and `Hot` dispatches a sync tenant whose lease lapsed or whose probe is due so a worker gets to do it (a parked sync tenant with no ready rows is never dispatched, so `EncryptKey` runs its due probe inline instead, 2026-09-25); `World.SetFetch(mode)` (timeline line, `metrics.SetKeyFetch`, `ResetPeaks`), snapshot `key_fetch`, `POST /v1/keyfetch {"mode"}`; the Slow KMS card's three mode buttons with the active one highlighted and a header badge while not `async`; the script's `slow_kms@naive` and `slow_kms@sync` runs set the mode before Start and flip back at 30 s | KEYFETCH |
| Naive mode (as built) | Stretch X1 ([NAIVE.md](NAIVE.md), approved 2026-09-24): `tenant.noBulkhead` with `Manager.SetNaive(id, on)` = pass-through plus no bulkheads; `call` skips the per-tenant cap and the provider semaphore for a naive tenant, still counts it in flight and still runs under `KMSTimeout`; `World.SetNaive(on)` flips every tenant; scenario `slow_kms_naive` (targets the slow band; phases `naive` `NaiveFor` 30 s and `leased` `NaiveLeasedFor` 30 s with the slow fault throughout, a timeline line at the flip and a summary of the naive half's readings against the leased half's peaks); `POST /v1/naive {"on"}`; the Slow KMS card's second button; the script allows L1 red for the run | NAIVE |
| No key cache (as built) | A sixth card, two runs ([NOCACHE.md](NOCACHE.md), approved 2026-09-23): `cmek.Manager.SetPassThrough(id, on)` puts a tenant in pass-through, where `EncryptKey`/`DecryptKey` each make one direct KMS call (no singleflight, the primitive handed to the caller once, the cache neither read nor filled, 0 hot DEKs), a Transient parks the tenant at once (`KEY_UNAVAILABLE`, detail `no cache: call failed`, never `RIDING_THROUGH`), `Hot` is `state == ACTIVE`, `Warm` is a no-op, and probes, backoff, bulkheads, the Deny row and the stale-OK guard are unchanged; `World.SetCache(provider|"", on)`; scenarios `no_cache` (one band, `NoCacheFor` + `NoCacheBlip` + `NoCacheAfter`, every provider at `NoCacheKMSP50/P99`) and `no_cache_surge` (everyone, `NoCacheSurgeLead` + `NoCacheSurgeFor` at `GlobalSurgeMult` + `NoCacheSurgeAfter`, targets the `GlobalSurgeAffectedTop` heaviest as `global_surge`), each posting its numbers to the timeline from `metrics.Last()` and the per-provider call counter `ProviderCallsPS`; the card's two starts; `DecryptKey` "never calls a KMS" now reads "unless the tenant is in pass-through" | NOCACHE |
| M5 as built | `Params.Lights` is `{S1, S2, S3, S4, L1, L4}` (no L2 light; L2 is chart 3); every light is always present, with green details `idle`, `grace`, `settling` and `global surge` where it is not judged: L1 is judged while a partial scenario runs (not in `global_surge`/`no_cache_surge`, where no tenant is unaffected), from `L1Grace` = 7 s (P99Window × SnapshotInterval + 2 s, amends 2 s) and turns red only after the p99 has been over its limit for `L1For` = 3 s (new; 0 = every pass) or on any healthy rejection; L4 is judged in the two global surges once the backlog has exceeded `Workers × ClaimBatch` for `L4Settle` = 7 s (amends 2 s, the same window rule); `Capacity` is a Param (620) with `Workers / mean sink latency` as the zero fallback; S3 judges `(tRevoke + Lease, tRestore]` with `sinceSeq` initialised from `Delivered(idx)` and a ring overrun reported as `coverage gap`, and posts the ground-truth line once per episode from `metrics.DetectedRevokedAt`; S4 keeps the sink bound `delivered ≤ sink ≤ delivered + claimed` from one `Store.Census(ctx, under)` that copies the ledger under `store.mu`; `traffic.Sink` keeps a per-tenant ring (`SinkRing`) under a per-tenant mutex; `Holder` has `Acquire`/`Ensure`/`Current`/`Reset`, each returning a release backed by a per-World `handlers` WaitGroup (`World.hold`), `Stop` waits ≤ `StopTimeout` for goroutines and handlers, `POST /v1/reset` runs outside `Ensure`; SSE frames are written under a `StreamWriteTimeout` deadline (`http.NewResponseController`) and `event: reconnect` at `StreamMaxAge`; `Expire` runs in the sweep every `ExpireInterval`; `check.Clock` gains `After`, `check.Health` may be nil; static files are served `Cache-Control: no-cache` | M5 |
...
| `scenarios.go` | 150 → 260 | M3 (runner + 2 surges) → M4 (3 more, targets, tail/recovery) → NOCACHE (2 more) | phase runner, seven scenarios on six cards, targets, control API |
...
- **M4**: each of the five cards reads within 90 s; healthy p99 line flat during slow-KMS and tenant-surge. **NOCACHE** (sixth card): the band run's blip parks the band within a second and the global run pins delivered under capacity; L2 red in both, L1 and L4 red in the global run, all green again in the tail.
