...
- **Slow KMS.** azure's latency rises to p50 400 ms, p99 3 s against a 500 ms timeout. azure cells flicker yellow, cold ones see slow ingest and a few 503s, in-flight calls to azure pin at the bulkhead, and the healthy p99 line stays flat. *Naive*: the same slow azure with the whole service on the naive design (no lease, cache or bulkheads) for 30 s: delivered collapses to ~20/s for everyone, the healthy p99 reaches ~25 s, heavy aws and gcp tenants get `503 backlog_full`, L1 goes red; then the switch flips back with azure still slow and delivered is at capacity within a second, the backlog gone in ~10 s.
